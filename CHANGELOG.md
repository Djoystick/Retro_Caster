...
- **Critical:** Закрыта уязвимость типа Argument Injection в модуле обрезки видео (FFmpeg). Внедрена строгая Regex валидация формата времени (`HH:MM:SS`), исключающая возможность передачи вредоносных системных флагов в `fluent-ffmpeg`.
- **High:** Усилена изоляция Electron. В конфигурации `BrowserWindow` принудительно включена песочница (`sandbox: true`).
- **Low:** Удалена небезопасная конструкция `eval()` в скриптах загрузки Telegram (заменено на нативный `require`).
